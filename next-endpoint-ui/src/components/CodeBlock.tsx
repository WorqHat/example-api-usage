...
export type CodeBlockFilesProps = HTMLAttributes<HTMLDivElement>;
...
export const CodeBlockFilename = ({
  ...
  // Prefer mapping by active language, then fall back to filename pattern
  const languageIcon =
    (activeValue && languageIconMap[activeValue.toLowerCase()]) ?? undefined;

  const filenameIcon = Object.entries(filenameIconMap).find(([pattern]) => {
  ...
  const Icon = icon ?? languageIcon ?? filenameIcon;
  if (value !== activeValue) {
    return null;
  }
  return (
    <div
      className="flex h-8 items-center gap-2 bg-secondary px-4 text-muted-foreground text-xs"
      {...props}
    >
...
export const CodeBlockSelectTrigger = ({
  ...
  <SelectTrigger
    className={cn(
      'h-7 w-fit border-none px-2 text-xs text-muted-foreground shadow-none flex items-center justify-start',
      className
    )}
    {...props}
  >
...
export const CodeBlockSelectContent = (
  props: CodeBlockSelectContentProps,
) => <SelectContent {...props} />;
...
export const CodeBlockSelectItem = ({
  ...
  const language =
    typeof props.value === 'string' ? props.value.toLowerCase() : undefined;
  const label =
    typeof children === 'string' ? children.trim() : String(children).trim();

  const languageIcon =
    (language && languageIconMap[language]) ?? undefined;
  ...
  const Icon = languageIcon ?? filenameIcon;

  return (
    <SelectItem
      className={cn('text-xs [&>span:first-child]:hidden', className)}
      {...props}
    >

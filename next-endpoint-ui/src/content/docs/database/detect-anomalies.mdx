...
<CodeBlock
  ...
  data={[
    {
      language: "javascript",
      filename: "detect-anomalies.js",
      code: `import Worqhat from 'worqhat';
...
detectAnomalies();`,
    },
    {
      ...
      code: `from worqhat import Worqhat
    import os
    ...
    func main() {
      ...
      response, err := client.DB.DetectAnomalies(context.TODO(), worqhat.DBDetectAnomaliesParams{
        ...
        Threshold: 0.8,
        Limit: 50,
        Environment: getEnvOrDefault("WORQHAT_ENVIRONMENT", "production"), // Defaults to production
      })
    ...
    {
      language: "bash",
      filename: "detect-anomalies.sh",
      code: `# Detect anomalies
...
>
  <CodeBlockHeader>
    <CodeBlockFiles>
      ...
      <CodeBlockFilename value="python">
        detect_anomalies.py
      </CodeBlockFilename>
      <CodeBlockFilename value="go">
        detect_anomalies.go
      </CodeBlockFilename>
      <CodeBlockFilename value="bash">
        detect-anomalies.sh
      </CodeBlockFilename>
    </CodeBlockFiles>
    ...
    <CodeBlockSelect>
      ...
      <CodeBlockSelectContent>
        <CodeBlockSelectItem value="javascript">
          detect-anomalies.js
        </CodeBlockSelectItem>
        <CodeBlockSelectItem value="python">
          detect_anomalies.py
        </CodeBlockSelectItem>
        <CodeBlockSelectItem value="go">
          detect_anomalies.go
        </CodeBlockSelectItem>
        <CodeBlockSelectItem value="bash">
          detect-anomalies.sh
        </CodeBlockSelectItem>
      </CodeBlockSelectContent>
    </CodeBlockSelect>
  ...
  <CodeBlockBody>
    <CodeBlockItem value="javascript">
...
try:
    ...
    <CodeBlockItem value="go">
      <CodeBlockContent language="go">
        {`package main
...
detectFraud();`,
    ...
    {
      language: "bash",
      filename: "detect-fraud.sh",
      code: `# Detect fraud with stricter parameters
curl -X POST "https://api.worqhat.com/db/detect-anomalies" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
...
checkDataQuality();`,
    ...
    func main() {
      ...
      // Check data quality with moderate parameters
      env := os.Getenv("WORQHAT_ENVIRONMENT")
      if env == "" {
        env = "production"
      }
      response, err := client.DB.DetectAnomalies(context.TODO(), worqhat.DBDetectAnomaliesParams{
    ...
    {
      language: "bash",
      filename: "data-quality.sh",
      code: `# Check data quality with moderate parameters
curl -X POST "https://api.worqhat.com/db/detect-anomalies" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
